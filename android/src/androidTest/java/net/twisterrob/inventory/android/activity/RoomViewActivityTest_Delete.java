...
public class RoomViewActivityTest_Delete {
	...
	@Test public void testDeleteMessageWithContents() throws IOException {
		App.db().createItem(DatabaseDTOTools.getRoot(roomID), Category.DEFAULT, TEST_ITEM, NO_DESCRIPTION);
	...
	@Test public void testDeleteMessageWithContentsMultiple() throws IOException {
		App.db().createItem(DatabaseDTOTools.getRoot(roomID), Category.DEFAULT, TEST_ITEM, NO_DESCRIPTION);
		App.db().createItem(DatabaseDTOTools.getRoot(roomID), Category.DEFAULT, TEST_ITEM_OTHER, NO_DESCRIPTION);

		testDeleteMessage();

		onView(isRoot()).check(matches(hasDescendant(withText(containsString(TEST_ITEM)))));
		onView(isRoot()).check(matches(hasDescendant(withText(containsString(TEST_ITEM_OTHER)))));
	}
	...
	@Test public void testDeleteConfirm() throws IOException {
		...
		assertThat(db.get(), not(hasInvProperty(TEST_ROOM)));
